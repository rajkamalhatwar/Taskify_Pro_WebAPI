        ...
        public async Task<IActionResult> SaveUser([FromBody] VMUserReg vMUserReg)
        {
            ...
            try
            {
                ...
                return res switch
                {
                    ...
                    2 => Ok(new { Message = "User updated successfully." }),
                    3 => Conflict(new { Message = "User with this email already exists." }),
                    4 => NotFound(new { Message = "User not found for update." }),
                    -99 => StatusCode(500, new { Message = "An unexpected error occurred." }),
                    _ => StatusCode(500, new { Message = "Unknown response from server." })
                };
            }
            catch (Exception ex)
            {
